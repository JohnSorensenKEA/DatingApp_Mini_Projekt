...
public class JDBCChatService {
    ...
    //Gets all users new conversations
    public ArrayList<ConversationPreview> getNewConversations(int userID){
        String selectStatement =
                "SELECT co.conversation_id, uu2.username, uu2.photo FROM users uu " +
                ...
                "LEFT JOIN messages me ON co.conversation_id = me.conversation_id " +
                "JOIN user_conversation_relations uc2 ON co.conversation_id = uc2.conversation_id " +
                "JOIN users uu2 ON uc2.user_id = uu2.user_id " +
                "WHERE uu.user_id = ? AND me.message_id IS NULL AND uu2.user_id != ? " +
                "GROUP BY co.conversation_id " +
                "ORDER BY co.conversation_id DESC";
